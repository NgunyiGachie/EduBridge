...
class InstructorByID(Resource):
    def get(self, id):
        ...
        """
        response_dict = Instructor.query.filter_by(id=id).first()
    ...
    def patch(self, id):
        ...
        """
        record = Instructor.query.filter_by(id=id).first()
        if not record:
            return make_response(jsonify({"error": "Instructor not found"}), 400)
    ...
    def delete(self, id):
        ...
        try:
            ...
            db.session.commit()
            response_dict = {"message": "Instructor successfully deleted"}
